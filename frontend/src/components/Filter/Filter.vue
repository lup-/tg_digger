            ...
            <v-list-item v-on="on" v-bind="attrs" #default="{ active }" dense>
                ...
                <v-list-item-content>
                    <v-row no-gutters align="center">
                        <v-col cols="12" md="6">
                            <v-list-item-title class="mb-2 mb-md-0">{{ item.text }}</v-list-item-title>
                        </v-col>
                        <v-col cols="12" md="6">
                            ...
                            <date-field v-else-if="item.type === 'date'"
                                ...
                                @mousedown.stop="noopFunction"
                            ></date-field>
                            <v-row v-else-if="item.type === 'number'">
                                <v-col cols="6" md="3">
                                    <v-select :items="operatorTypes" v-model="operators[item.id]"
                                        dense
                                        outlined
                                        hide-details
                                        background-color="white"
    ...
    methods: {
        ...
        emitFilter() {
            let filter = this.activeFields.reduce((buildFilter, fieldId) => {
                ...
                if (isNumber) {
                    let value = {}
                    let operator = this.operators[fieldId];
                    if (!operator) {
                        operator = this.defaultOperator;
                    }

                    value[operator] = parseInt(this.values[fieldId]);
                    buildFilter[fieldId] = value;
                }
                else {
        ...
        textValues(fieldId) {
            ...
            else if (fieldParams.type === 'number') {
                let operator = this.operators[fieldId] || this.defaultOperator;
                let operatorParams = this.operatorTypes.find(data => data.value === operator);
                let operatorText = operatorParams ? operatorParams.text : '=';
        ...
        parseFilter() {
            ...
            for (let fieldId in this.value) {
                let fieldParams = this.fields.find(field => field.id === fieldId);
                let isNumber = fieldParams && fieldParams.type && fieldParams.type === 'number';

                if (isNumber) {
                    let operator = Object.keys(this.value[fieldId])[0];
                    let value = this.value[fieldId][operator];

                    this.operators[fieldId] = operator;
                    this.values[fieldId] = value;
                }
                else {
                    this.values[fieldId] = this.value[fieldId];
                }
            }
        },
